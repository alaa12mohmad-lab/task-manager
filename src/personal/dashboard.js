...
export function renderCounts(){
  ...
  const sel=document.getElementById('filter-emp');const cur=sel.value;
  sel.innerHTML=`<option value="">كل الموظفين</option>`+state.emps.map(e=>`<option value="${e.id}" ${cur===e.id?'selected':''}>${esc(e.name)}</option>`).join('');
}
