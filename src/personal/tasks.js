...
import { esc, fmtDate, col, uid, empById, isOverdue, isSuperAdmin, initials } from '../core/utils.js';
...
export function openTaskModal(id = null, prefillDate = null) {
  ...
  if (isEdit) {
    ...
    document.getElementById('f-emp').value = t.empId || '';
...
export async function saveTask() {
  ...
  try {
    if (state.editTaskId) {
      await col('tasks').doc(state.editTaskId).update({ title, desc, empId, cat, status, priority, due, notes });
      toast('تم تحديث المهمة ✓', 'ok');
    } else {
      const id = uid();
      await col('tasks').doc(id).set({ id, title, desc, empId, cat, status, priority, due, notes, created: Date.now() });
...
export function openTaskDetail(id) {
  const t = state.tasks.find(x => x.id === id);
  const emp = state.emps.find(e => e.id === t.empId);
  const sL = { pending: 'معلّقة', wip: 'قيد التنفيذ', done: 'منتهية', cancelled: 'ملغية' };
  const pL = { high: 'عالية 🔴', medium: 'متوسطة 🟡', low: 'منخفضة 🟢' };
  const sBg = emp ? emp.color : '#4f8ef7';
  const rows = [
    ['الموظف المسؤول', emp ? `<span style="display:flex;align-items:center;gap:7px"><span style="background:${sBg};width:22px;height:22px;border-radius:50%;display:inline-flex;align-items:center;justify-content:center;font-size:.6rem;font-weight:700;color:#fff">${initials(emp.name)}</span>${esc(emp.name)} — ${esc(emp.role || '')}</span>` : '—'],
...
export function renderTasks() {
  ...
  const empFilt = document.getElementById('filter-emp').value;
  ...
  let list = state.tasks.filter(t => {
    ...
    if (empFilt && t.empId !== empFilt) return false;
    if (search) {
      const emp = empById(t.empId);
      return t.title.toLowerCase().includes(search) || (t.desc || '').toLowerCase().includes(search) || (t.cat || '').toLowerCase().includes(search) || (emp && emp.name.toLowerCase().includes(search));
    }
    return true;
  });
  list.sort((a, b) => {
    ...
    if (sort === 'emp') return (empById(a.empId)?.name || '').localeCompare(empById(b.empId)?.name || '', 'ar');
    return 0;
  });
  ...
  c.innerHTML = list.map(t => {
    const emp = empById(t.empId);
    const av = emp ? `<div style="width:32px;height:32px;border-radius:50%;background:${emp.color};display:flex;align-items:center;justify-content:center;font-size:.72rem;font-weight:700;color:#fff;flex-shrink:0;border:2px solid var(--border2)">${initials(emp.name)}</div>` : `<div style="width:32px;height:32px;border-radius:50%;background:var(--s3);display:flex;align-items:center;justify-content:center;font-size:.8rem;color:var(--muted);flex-shrink:0;border:2px solid var(--border)">؟</div>`;
    const empChip = emp ? `<span style="display:flex;align-items:center;gap:4px;font-size:.72rem;color:${emp.color};background:${emp.color}18;border:1px solid ${emp.color}40;border-radius:20px;padding:2px 8px 2px 4px"><span style="background:${emp.color};width:14px;height:14px;border-radius:50%;display:inline-flex;align-items:center;justify-content:center;font-size:.5rem;font-weight:700;color:#fff">${initials(emp.name)}</span>${esc(emp.name)}</span>` : '';
    ...
    return `<div class="task-card" data-s="${t.status}"><div class="task-check" onclick="toggleStatus('${t.id}')">${ck}</div>${av}<div class="task-body"><div class="task-title">${esc(t.title)}</div>${t.desc ? `<div class="task-desc">${esc(t.desc)}</div>` : ''}<div class="task-meta"><span class="badge b-${t.status}">${statusLabels[t.status]}</span><span class="badge b-${t.priority}">${priLabels[t.priority]}</span>${empChip}${dl ? `<span class="${dc}">${dl}</span>` : ''}${t.cat ? `<span class="task-cat">🏷 ${esc(t.cat)}</span>` : ''}</div></div><div class="task-actions"><button class="icon-btn" title="التعليقات" onclick="openComments('${t.id}',null,'${esc(t.title).slice(0, 40)}')">💬</button><button class="icon-btn" onclick="openTaskDetail('${t.id}')">👁</button><button class="icon-btn" onclick="openTaskModal('${t.id}')">✏️</button>${isSuperAdmin() ? `<button class="icon-btn del" onclick="deleteTask('${t.id}')">🗑</button>` : ''}</div></div>`;
  }).join('');
}
